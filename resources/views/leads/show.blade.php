...
@section('content')
    ...
    <div class="box box-info">
        <div class="box-header with-border">
			...
			@can('edit-lead')<a href="{!! url('/leads/'.$lead_detail['id'].'/edit' ); !!}" class="btn btn-info"><li
                            class="fa fa-pencil"></li> Edit</a>@endcan
                @if($lead_detail->approvestatus==0 or $lead_detail->approvestatus==2)
                    @can('approve-reject-lead')<a href="{!! url('/leads/approve/'.$lead_detail['id'].'' ); !!}"
                                                  class="btn btn-success"><li
                                class="fa fa-check"></li> Approve</a>@endcan
                @endif
                @if($lead_detail->approvestatus==0 or $lead_detail->approvestatus==1)
                    @can('approve-reject-lead')<a href="{!! url('/leads/reject/'.$lead_detail['id'].'' ); !!}"
                                                  class="btn btn-danger"><li class="fa fa-ban"></li> Reject</a>@endcan
                @endif
                @if($lead_detail->istraininglead==0)
                    @can('for-training-lead')<a href="{!! url('/leads/fortraining/'.$lead_detail['id'].'' ); !!}"
                                                class="btn btn-primary"><li
                                class="fa fa-book"></li> For Training</a>@endcan
                @else
                    @can('for-training-lead')<a href="{!! url('/leads/removefromtraining/'.$lead_detail['id'].'' ); !!}"
                                                class="btn btn-primary"><li class="fa fa-book"></li> Remove From Training</a>@endcan
                @endif
                ...
                @can('create-proposal')<a href="{!! url('leads/createproposal/'.$lead_detail['id'].'' ); !!}"
                                          class="btn btn-success"><li class="fa fa-plus"></li> Proposal</a>@endcan
                ...
                @can('close-this-lead')
                    @if(!$lead_detail->closed)
                        <a href="{!! url('leads/close/'.$lead_detail->user_id.'/'.$lead_detail->id.''); !!}"
                           class="btn btn-danger"> Close this Lead</a>
                    @else
                ...
                <div class="col-md-12">
                    ...
                    <table class="table table-striped">
                        <tr>
                            ...
                            <td width="75%">{{$lead_detail->user->fname}} {{$lead_detail->user->lname}} <input
                                        type="hidden" value="{{$lead_detail->id}}" id="lead_id"></td>

                        </tr>
                        <tr>
                            <td><b>Phone Number </b></td>
                            <td>
                                <a href="tel:{{$lead_detail->user->phonenumber}}">{{$lead_detail->user->phonenumber}}</a>
                            </td>
                        </tr>
                        <tr>
                            <td><b>Mobile Number </b></td>
                            <td>
                                <a href="tel:{{$lead_detail->user->mobilenumber}}">{{$lead_detail->user->mobilenumber}}</a>
                            </td>
                        </tr>
                        <tr>
                ...
                <div class="col-md-12">
                    ...
                    <table class="table table-striped">
                        ...
                        <tr>
                            ...
                            <td width="75%">Residential: <b>{{$lead_detail->residential=== 1 ? "Yes" : "No"}}</b><br>
                                Commercial: <b>{{$lead_detail->commercial=== 1 ? "Yes" : "No"}}</b></td>
                        </tr>

                        <tr>
                            <td width="25%"><b>Interested In</b></td>
                            <td width="75%">Cash: <b>{{$lead_detail->cash=== 1 ? "Yes" : "No"}}</b> <br> Installment:
                                <b>{{$lead_detail->installment=== 1 ? "Yes" : "No"}}</b></td>
                        </tr>
    ...
    <div class="box box-success">
        <div class="box-header with-border">
            ...
            <button class='btn btn-primary edit-modal pull-right' data-toggle='modal' data-target='#myModal'>Add
                Status
            </button>
        </div>
        <!-- /.box-header -->
                ...
                <div class="modal-body" id="modalbody">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-control"
                               value="{{$lead_detail->user->fname}} {{$lead_detail->user->lname}}" readonly>
                    </div>
                    <div class="form-group">
                        <label>Status</label>
...
{{--                            <option value="6">Interested in Webinar</option>--}}
                    ...
                    <div class="form-group">
                        <label>Message</label>
                        <textarea class="form-control" rows="3" placeholder="Enter ..." name="notes" id="notes"
                                  required></textarea>
                    </div>

                </div>
                <div class="modal-footer" id="modalfooter">
                    <input type="hidden" name="leadid" id="leadid" class="form-control" value="{{$lead_detail->id}}"
                           readonly required="required">
                    <input type="hidden" name="show_page" id="show_page" class="form-control" value="show_page"
                           readonly>
    ...
    <div class="box box-info">
                    ...
                    @foreach($recordings as $recording)
                        <tr>
                            ...
                            <td>
                                @if($recording->link)
                                    <audio controls>
                                        <source src="{{$recording->link=="" ? "" : "$recording->link"}}"
                                                type="audio/mpeg">
                                ...
                                @elseif($recording->recording_file && Storage::disk('local')->exists('public/leads_assets/recordings/'.$recording->recording_file))
                                    @if(File::extension('public/leads_assets/recordings/'.$recording->recording_file)=="mp4")
                                        <video controls style="width: 350px;">
                                            <source src="{{Storage::disk('local')->url('public/leads_assets/recordings/'.$recording->recording_file)}}"
                                                    type="audio/mpeg">
                                            Your browser does not support the audio element.
                                        </video>
                                    @else
        ...
        <div class="box-footer clearfix" style="">
            <div>
                <a href="{!! url('/recordings/'.$lead_detail['id']); !!}" class="pull-right btn btn-info"
    ...
    <div class="box box-success">
        ...
        <div class="box-footer clearfix" style="">
            <div>
                <a href="{!! url('/appointments/'.$lead_detail['id']); !!}" class="pull-right btn btn-info"
                   style="margin-top:5px;">View All</a>
                @can('create-appointment')<a href="{!! url('leads/createappointments/'.$lead_detail['id'].'' ); !!}"
            ...
            @if(count($proposals) > 0)
                <table id="nofeaturesproposal" class="display responsive" style="width:100%;">
                    ...
                    @foreach($proposals as $proposal)
                        <tr>
                            <td><b>{{$proposal->title}}</b> <small> @ {{$proposal->created_at->format('d-m-Y')}}</small><br>{{$proposal->note}}
                            </td>
                            <td>
                                @if($proposal->docfile)
                                    @if(Storage::disk('local')->exists('public/leads_assets/proposal/'.$proposal->docfile))
                                        <a href="{{Storage::disk('local')->url('public/leads_assets/proposal/'.$proposal->docfile)}}"
                                    ...
                                    @endif
                                @else
                                    @can('upload-proposal')<a
                                            href="{!! url('leads/uploadproposal/'.$lead_detail['id'].'/'.$proposal['id'].'' ); !!}"
                                            class="btn btn-danger" title="Upload Proposal File">
                            ...
                            <td>
                                @if(!$proposal->docfile)
                                    @can('upload-proposal')<a
                                            href="{!! url('leads/uploadproposal/'.$lead_detail['id'].'/'.$proposal['id'].'' ); !!}"
                                            class="btn btn-primary" title="Upload Proposal"><i
                                                class="fa fa-paperclip"></i> </a>@endcan
                                @endif
                                @can('edit-proposal')<a
                                        href="{!! url('/leads/edit_proposal/'.$proposal['id'].'/'.$lead_detail['id'].''); !!}"
                                        class="btn btn-success" title="Edit"><i class="fa fa-edit"></i> </a>@endcan
        ...
        <div class="box-body" style="">
            @if(count($docs) > 0)
                <table id="nofeaturesdocs" class="display responsive" style="width:100%;">
                    ...
                    @foreach($docs as $doc)
                        <tr>
                            <td><b>{{$doc->title}}</b> <small>@ {{$doc->created_at->format('d-m-Y')}}</small>
                                <br>{{$doc->note}}</td>
                            <td>
                                @if($doc->docfile)

                                    @if(Storage::disk('local')->exists('public/leads_assets/docfiles/'.$doc->docfile))
                                        <a href="{{Storage::disk('local')->url('public/leads_assets/docfiles/'.$doc->docfile)}}"
                                           target="_blank" class="btn btn-info">
                                            <li class="fa fa-file"></li>
                                            View</a>
                                    @else
