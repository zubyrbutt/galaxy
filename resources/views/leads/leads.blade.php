...
@section('content')
    ...
    @can('search-leads')
        <div class="row">
            <div class="col-md-12">
                <form class="form-horizontal" action="{!! url('/leads/search'); !!}" method="post"
                    ...
                    <div class="box box-success collapsed-box">
                        ...
                        <div class="box-body" style="display: none;">
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <select name="agentid" class="form-control select2 select2-hidden-accessible"
                                ...
                                <select name="countryId" class="form-control select2 select2-hidden-accessible"
                                        multiple="" data-placeholder="Select a Satff" style="width: 100%;" tabindex="-1"
                                        aria-hidden="true">
                                    @can('show-all-leads')
                                        <option value="all">All</option>@endcan
                            ...
                            @foreach($leads as $lead)
                                ...
                                @can('delete-lead')
                                    <!-- For Delete Form begin -->
                                        <form id="form{{$lead['id']}}"
                                              action="{{action('LeadController@destroy', $lead['id'])}}" method="post">
                                            @csrf
                                    ...
                                    <td>
                                        @can('show-lead')
                                            <a href="{!! url('/leads/'.$lead['id'] ); !!}" class="btn btn-primary"
                                        ...
                                        @can('status-lead')
                                            @if ($lead['status'] === 1)
                                                <a href="{!! url('/leads/deactivate/'.$lead['id']); !!}"
                                                   class="btn btn-warning" title="Deactivate"><i
                                                            class="fa fa-times"></i> </a>
                                            @else
                                                <a href="{!! url('/leads/active/'.$lead['id']); !!}"
                                                   class="btn btn-info" title="Active"><i class="fa fa-check"></i> </a>
                                            @endif
                                        @endcan
                                        @can('delete-lead')
                                            <button class="btn btn-danger"
                                                    onclick="archiveFunction('form{{$lead['id']}}')"><i
